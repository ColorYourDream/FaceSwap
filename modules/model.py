...
class FullModel(torch.nn.Module):
    ...
    def __init__(self, segmentation_module, reconstruction_module, train_params):
        ...
        if sum(self.loss_weights['perceptual']) != 0:
            ...
            self.lightCNN = LightCNN_29Layers_v2(num_classes=80013)
            self.cnnCheckpoint = torch.load("LightCNN/LightCNN_29Layers_V2_checkpoint.pth.tar")

            self.new_model_state = {}

            for key in self.cnnCheckpoint['state_dict'].keys():
                self.new_model_state[key[7:]] = self.cnnCheckpoint['state_dict'][key]

            self.lightCNN.load_state_dict(self.new_model_state, strict=False)
            self.lightCNN = self.lightCNN.cuda()

            for param in self.lightCNN.parameters():
                param.requires_grad = False

    def resizeGrayscale(self, x):
        x = F.interpolate(x, 128)
        return x
    ...
    def forward(self, x):
        ...
        if sum(self.loss_weights['perceptual']) != 0:
            value_total = 0
            for scale in self.scales:
                resizedGrayscaleGenerated = self.resizeGrayscale(pyramide_generated['prediction_' + str(scale)])
                resizedGrayscaleReal = self.resizeGrayscale(pyramide_real['prediction_' + str(scale)])

                featuresX, outputX = self.lightCNN(resizedGrayscaleGenerated)
                featuresY, outputY = self.lightCNN(resizedGrayscaleReal)

                value = torch.abs(featuresX - featuresY.detach()).mean()
                value_total += self.loss_weights['perceptual'][0] * value
                value = torch.abs(outputX - outputY.detach()).mean()
                value_total += self.loss_weights['perceptual'][1] * value
            loss_values['perceptual'] = value_total
